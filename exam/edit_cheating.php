...
if (isset($_POST['add'])) {
    ...
    $fields = array(
        ...
        'الفصل الدراسي' => 'semester', 'العام الدراسي' => 'year', 'المقرر' => 'course_no', 'الشعبة' => 'section_seq', 'واقعة الغش والأدوات المستخدمة' => 'cheat_case',
        'المشرف' => 'monetor_member_id1', 'المراقب' => 'monetor_member_id2', 'رئيس اللجنة' => 'com_member_id1', 'الطالب' => 'student_id','عضو اللجنة' => 'com_member_id2'
    );
    ...
    $com_member_id2 = mysqli_real_escape_string($conn, trim($_POST['com_member_id2']));
    ...
    $cheat_case = mysqli_real_escape_string($conn, trim($_POST['cheat_case']));
    $monetor_member_id1 = mysqli_real_escape_string($conn, trim($_POST['monetor_member_id1']));
    $monetor_member_id2 = mysqli_real_escape_string($conn, trim($_POST['monetor_member_id2']));
    ...
    if (empty($errors)) {
        if ($filename == '') {
            $sql = "UPDATE cheating SET `campus_no`='$campus_no',`com_member_id2`='$com_member_id2',`room_no`=$room_no,`date`='$date',`time`='$time',`period`='$period',`year`='$year',
`semester`='$semester',`course_no`='$course_no',`section_seq`='$section_seq',`cheat_case`='$cheat_case',`monetor_member_id1`='$monetor_member_id1',`monetor_member_id2`='$monetor_member_id2'
,`com_member_id1`='$com_member_id1',`student_id`='$student_id' WHERE cheat_id=$id";
        }else{
            $sql = "UPDATE cheating SET `campus_no`='$campus_no',`com_member_id2`='$com_member_id2',`room_no`=$room_no,`date`='$date',`time`='$time',`period`='$period',`year`='$year',
`semester`='$semester',`course_no`='$course_no',`section_seq`='$section_seq',`cheat_case`='$cheat_case',`monetor_member_id1`='$monetor_member_id1',`monetor_member_id2`='$monetor_member_id2'
,`com_member_id1`='$com_member_id1',`doc`='$filename',`student_id`='$student_id' WHERE cheat_id=$id";
        }
...
<div class="row">
    ...
    <div class="col-lg-6" style="text-align: right" dir="rtl">
        <form action="" method="post" enctype="multipart/form-data">
            ...
            ?>
            <?php
            $sqlMain = "select * from `cheating` WHERE cheat_id='$id' limit 1 ";
            ...
            <div class="form-group">
                <label>واقعة الغش والأدوات المستخدمة</label>
                <textarea name="cheat_case" rows="3" type="text" class="form-control"><?php if (isset($_POST['cheat_case'])) {
                        echo $_POST['cheat_case'];
                    }else{ echo $row5['cheat_case']; } ?></textarea>
            </div>


            <div class="form-group">
                <label>المشرف</label>
                <select type="text" name="monetor_member_id1" class="form-control">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            if (isset($_POST['monetor_member_id1'])) {
                                if ($_POST['monetor_member_id1'] == $row['instructor_id']) {
                                    echo 'selected';
                                }
                            }else{
                                if($row5['monetor_member_id1'] == $row['instructor_id']){
                                    echo  'selected';
                                }
                            }
            ...
            <div class="form-group">
                <label>المراقب</label>
                <select type="text" name="monetor_member_id2" class="form-control">
                    <option value=""> -- اختيار المراقب --</option>
                    <?php
                    $sql = "select * from `instructors` ";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ?>
                            <option value="<?php echo $row['instructor_id'] ?>" <?php
                            if (isset($_POST['monetor_member_id2'])) {
                                if ($_POST['monetor_member_id2'] == $row['instructor_id']) {
                                    echo 'selected';
                                }
                            }else{
                                if($row5['monetor_member_id2'] == $row['instructor_id']){
                                    echo  'selected';
                                }
                            }
                            ?>><?php echo $row['instructor_name'] ?></option>
            ...
            <div class="form-group">
                ...
                <div class="form-group">
                    <label>عضو اللجنة</label>
                    <select type="text" name="com_member_id2" class="form-control">
                        <option value=""> -- اختيار عضو اللجنة --</option>
                        <?php
                        $sql = "select * from `instructors` ";
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ?>
                                <option value="<?php echo $row['instructor_id'] ?>" <?php
                                if (isset($_POST['com_member_id2'])) {
                                    if ($_POST['com_member_id2'] == $row['instructor_id']) {
                                        echo 'selected';
                                    }
                                }else{
                                    if($row5['com_member_id2'] == $row['instructor_id']){
                                        echo  'selected';
                                    }
                                }
                                ?>><?php echo $row['instructor_name'] ?></option>
            ...
            <div class="form-group">
                <label for="exampleInputEmail1" class="text-dark font-weight-bold">المرفقات</label>
                <input type="file"  name="picture" class="form-control-file" id="exampleInputFile"  aria-describedby="fileHelp">
                <a href="img/<?php echo $row5['doc']; ?>" download="">تنزيل</a>
            </div>
